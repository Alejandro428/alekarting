...
<!-- Para formatear las fechas, debe cargarse después de Jquery -->
...
<script>
    ...
    $.extend(true, $.fn.dataTable.defaults, {
        autoWidth: false, // no ajustar el ancho de las columnas automáticamente
        ...
        layout: {
            ...
            topEnd: 'search', // que elementos de la paginación queremos que aparezcan    
            bottomStart: 'info',
    ...
    // Función para limpiar el valor de una celda de la tabla de empleados <<<---- ES UN EJEMPLO NO SE UTILIZA
    function limpiarMoneda(valor) {
        // Eliminar el símbolo "€" y reemplazar la coma por un punto (para formato decimal correcto)
        return parseFloat(valor.replace('€', '').replace(',', '.'));
    }

     // Función para dar formato europeo a una fecha y hora
     function formatoFechaEuropeo(fechaAmericana) {
        if (!fechaAmericana) return ""; // Si la fecha está vacía, devuelve una cadena vacía
    ...
    // Función para dar formato europeo a una fecha y hora en formato americano - devuelve solo la fecha omitiendo la hora
    function formatoFechaEuropeoSoloFecha(fechaAmericana) {
        if (!fechaAmericana) return ""; // Si la fecha está vacía, devuelve una cadena vacía

        const [fecha, hora] = fechaAmericana.split(" "); // Separamos fecha y hora
        const [year, month, day] = fecha.split("-"); // Separamos la parte de la fecha

        return `${day}-${month}-${year}`.trim(); // Formato europeo
    }
    // 🔹 Ejemplo de uso:
    //    console.log(formatoFechaEuropeoSoloFecha("2025-02-26 14:30:15")); // Salida: 26-02-2025 
    //    console.log(formatoFechaEuropeoSoloFecha("2025-02-26")); // Salida: 26-02-2025
...
<script>
    ...
    class FormValidator {
        ...
        // clearValidation(): Limpia los estados de validación
        ...
        initialize() {
            ...
            // Este punto trabaja sobre cada campo del formulario y trabaja según cada tecla apretada
        ...
        validateForm(e) {
            ...
            //Por cada campo comprueba si es requerido y si es requerido hace la comprobación con el validateField y se es errorneo
            ...
            /*
               1.- Object.keys(this.fields).forEach(field => {
                ...
                Object.keys(this.fields):
                  ...
                  En este caso, this.fields es el objeto de configuración que contiene los campos del formulario y sus reglas de validación.
                ...
                .forEach(field => { ... }):
                      Recorre el array de claves (nombre, email, telefono) y ejecuta la función definida dentro del forEach para cada clave.
                      La variable field representará cada clave durante la iteración.
            */
            ...
            Object.keys(this.fields).forEach(field => {
                ...
                const input = this.form.querySelector(#${field});
                  ...
                  Busca un elemento dentro del formulario cuyo id coincida con el nombre de la clave actual (field).
                                Por ejemplo:
                                Si field es 'nombre', esto seleccionará el campo:
                                             <input id="nombre">
                El resultado es que input será una referencia al elemento del DOM correspondiente.
                */
                ...
                /*
                              ...
                              Esta línea tiene dos condiciones que se deben cumplir para ejecutar el bloque del if:
                                            Condición 1: this.fields[field].required
                                                 this.fields[field]: Accede a la configuración del campo actual dentro del objeto fields.
                                                 .required: Comprueba si el campo está marcado como obligatorio.
                                            ...
                                            Condición 2: !this.validateField(input)
                                                    this.validateField(input): Llama al método validateField, que valida el contenido del campo (según las reglas definidas en fields) y devuelve true si es válido o false si no lo es.
                                                       /* !this.validateField(input):
                                            Si el campo no pasa la validación (es inválido), esta condición será true.
                */

                /* Campo requerido = true -- Pasa la validacion = isValid= true
                /* Campo requerido = true -- NO pasa la validación = isValid = false
        ...
        validateField(input) {
            //this.fields[input.id]: Busca en el objeto fields la configuración asociada al campo que se está validando.
            ...
            /*value: Es el valor actual del campo de entrada, que se obtiene con input.value.
                regex.test(value): Verifica si el valor cumple con el patrón de la expresión regular:
                Retorna true si el valor cumple con el patrón.
            ...
            /*
            classList.toggle(): Añade o elimina una clase CSS del elemento, dependiendo del segundo parámetro:
            Si el segundo parámetro es true, la clase se añade.
