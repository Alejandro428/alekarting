$(document).ready(function () {
    ...
    $('#modalEmpleado').on('shown.bs.modal', function() {
        $('#fecha_publicacion').datepicker('refresh');
    });
...
$(document).on('click', '#btnAyudaNoticias', function (event) {
  ...
  $('#modalAyudaNoticias').modal('show');
    ...
    // VALIDADOR DEL FORMULARIO DE CATEGORIAS 
    var formCategoria = new FormValidator('formCategoria', {
        nombre_categoria: {
            required: true
        }
    });
    ...
    //         DATATABLES             //
    ///////////////////////////////////
    // CONFIGURACIÓN DEL DATATABLE PARA GESTIONAR LAS NOTICIAS COMO ADMIN
    var datatable_noticiaConfig = {
        ...
        columns: [
            ...
            { name: 'nombre_usuario', data: 'nombre_usuario' }, // Columna 1: NOMBRE USUARIO
            { name: 'titulo', data: 'titulo' }, // Columna 2: TÍTULO
            { name: 'nombre_categoria', data: 'nombre_categoria' }, // Columna 3: NOMBRE DE CATEGORÍA
            { name: 'fecha_publicacion', data: 'fecha_publicacion' }, // Columna 4: FECHA DE PUBLICACIÓN
            { name: 'imagen', data: 'imagen' },  // Columna 5: IMAGEN
            { name: 'editar', data: null, defaultContent: '' },  // Columna 6: EDITAR
            { name: 'eliminar', data: null, defaultContent: '' }   // Columna 7: ELIMINAR
        ],
        ...
        ajax: {
            url: base_url + "EmpleadoNoticias/obtenerTodasLasNoticias",
            type: 'GET',
            dataSrc: 'data'
        },
        ...
        swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.post(base_url + "EmpleadoNoticias/eliminarNoticias/" + id, function(data) {
                    ...
                    // NOTIFICACIÓN PARA AVISAR AL ADMIN DE QUE SE HA ELIMINADO LA NOTICIA
                    swal.fire(
                        'Desactivado',
...
$(document).on('click', '#btnsalvar', async function(event) {
    ...
    function resetearModalCategoria() {
        $('#formCategoria')[0].reset();
        $('#formCategoria').find('input[name="id_categoria"]').val("");
        $('#nombre_categoria').removeClass('is-valid is-invalid');
        $('#divNombreCategoria').show();
        $('#divSelectCategoria').hide();
        $('#btnSalvarCategoria').show().text(' Guardar');
        $('#btnConfirmarEliminar').hide();
        $('[data-bs-dismiss="modal"]').html('<i class="fas fa-times"></i> Cerrar');
    }

    ///////////////////////////////////////
    //    INICIO ZONA CREA CATEGORÍA    //
    /////////////////////////////////////

    // EVENTO DE CLICK PARA CREAR LA CATEGORÍA
    $(document).on('click', '#btnCrearCategoria', function (event) {
        event.preventDefault();
        // SE ACLARA QUE SE ESTÁ CREANDO POR AHORA LA CATEGORÍA
        modoActual = 'crear';
        // SE RESETEA EL MODAL, POR VALIDACIONES O MOSTRAR CAMPOS PREVIAMENTE
        resetearModalCategoria();
        $('#mdltituloCategoria').text('Nuevo registro de categoría');
        // MOSTRAR EL MODAL
        $('#modalCategoria').modal('show');
    });

    ///////////////////////////////////////
    //    FIN ZONA CREA CATEGORÍA    //
    /////////////////////////////////////

    ///////////////////////////////////////////
    //  CHANGE PARA SELECT CATEGORIA //
    ///////////////////////////////////////////

    // EVENTO DE CAMBIO AL ESCOGER OTRO SELECT DE LA CATEGORÍA
    $(document).on('change', '#select_categoria', function() {
        const opcion = $(this).find('option:selected');
        const id = opcion.val();
        const nombre = opcion.text();
        // RECOGER LA OPCIÓN, SU ID Y NOMBRE
        
        $('#formCategoria').find('input[name="id_categoria"]').val(id);
        
        // SI SE ESTÁ EDITANDO, SE MUESTRAN X CAMPOS, SINO, SE OCULTAN OTROS
        if (modoActual === 'editar') {
            if (id) {
                $('#divNombreCategoria').show();
                $('#nombre_categoria').val(nombre);
            } else {
                $('#divNombreCategoria').hide();
    ...
    ///////////////////////////////////////
    //    INICIO ZONA EDITAR CATEGORÍA    //
    /////////////////////////////////////
    // EVENTO AL HACER CLICK PARA EDITAR LA CATEGORÍA
    $(document).on('click', '#btnEditarCategoria', function(evento) {
        evento.preventDefault();
        // SE PONE EN MODO EDITAR LA CATEGORÍA
        modoActual = 'editar';
        resetearModalCategoria();
        $('#mdltituloCategoria').text('Editar categoría');
        $('#modalCategoria').modal('show');
        
        // SE MUESTRAN VARIOS CAMPOS
        $('#divSelectCategoria').show();
        $('#divNombreCategoria').hide();
        
        cargarCategorias('#select_categoria');
    });

     ///////////////////////////////////////
    //    INICIO ZONA CREAR/EDITAR CATEGORÍA//
    /////////////////////////////////////
    // EVENTO AL HACER CLICK EN SALVAR CATEGORÍA
    $(document).on('click', '#btnSalvarCategoria', async function(evento) {
        evento.preventDefault();
        
        const categoriaId = $('#formCategoria').find('input[name="id_categoria"]').val();
        const categoriaNombre = ($('#nombre_categoria').val() || '').trim();
        
         // SE RECOGE EL ID Y EL NOMBRE DE LA CATEGORÍA, Y DESPUÉS
         // SI SE ESTÁ EDITANDO Y NO HAY CATEGORÍA SELECCIONADA, NO SE PUEDE CONTINUAR
    if (modoActual === 'editar' && !categoriaId) {
        toastr.error("Debe seleccionar una categoría para editar", "Error");
        return;
    }

     // VALIDACIÓN DEL FORMULARIO
     const isFormValid = formCategoria.validateForm(evento);
     if (!isFormValid) {
         toastr.error("Por favor, corrija los errores en el formulario.", "Error de Validación");
         return;
     }
        
        // SE HACE UN AJAX PARA VERIFICAR QUE NO EXISTA PREVIAMENTE LA CATEGORÍA QUE SE QUIERE CREAR/EDITAR
        try {
            const verificacion = await $.ajax({
                url: base_url + "Categorias/verificarCategoria",
                type: "POST",
                data: {
            ...
            // EN CASO DE EXISTIR PREVIAMENTE, SE NOTIFICA AL ADMIN Y NO SE DEJA CREAR LA CATEGORÍA
            if(verificacion.existe) {
                toastr.error(verificacion.mensaje || "La categoría ya existe", "Error");
                $('#nombre_categoria').addClass('is-invalid');
                return;
            }
            
            // AJAX PARA CREAR/EDITAR LA CATEGORÍA
            const respuesta = await $.ajax({
                url: categoriaId 
                    ? base_url + "Categorias/editarCategoria" 
                    : base_url + "Categorias/crearCategoria",
                type: "POST",
                data: {
                    nombre_categoria: categoriaNombre,
                    id: categoriaId || null
            ...
            if(error.status === 409) {
                toastr.error("Conflicto: " + (error.responseJSON?.mensaje || "La categoría ya existe"), "Error");
                $('#nombre_categoria').addClass('is-invalid');
            } else {
     ...
     /////////////////////////////////////////
    //    FIN ZONA CREAR CATEGORÍA    //
    /////////////////////////////////////

    /////////////////////////////////////////
    //    INICIO ZONA ELIMINAR CATEGORÍA //
    /////////////////////////////////////

    // EVENTO AL HACER CLICK EN ELIMINAR CATEGORÍA
    $(document).on('click', '#btnEliminarCategoria', function(evento) {
        evento.preventDefault();
        // SE PONE EN MODO ELIMINAR
        modoActual = 'eliminar';
        
        resetearModalCategoria();
        $('#mdltituloCategoria').text('Eliminar categoría');
        $('#modalCategoria').modal('show');
        
        // SE MUESTRA SOLO EL SELECT, PARA SELECCIONAR QUE CATEGORÍA SE QUIERE ELIMINAR
        $('#nombre_categoria').val('');
        $('#select_categoria').val('');
        $('#divSelectCategoria').show();
        $('#divNombreCategoria').hide();
        $('#btnSalvarCategoria').hide();
    ...
    // EVENTO DE CLICK AL DARLE A CONFIRMAR ELIMINAR
    $(document).on('click', '#btnConfirmarEliminar', async function() {
        // SE RECOGE EL ID DE LA CATEGORÍA QUE SE QUIERE ELIMINAR
        const id = $('#formCategoria').find('input[name="id_categoria"]').val();
        // SI NO HAY ID ESCOGIDO, SE RETORNA SIN HACER NADA CON ERROR 
        if (!id) {
            toastr.error("No se ha seleccionado ninguna categoría", "Error");
            return;
        }
        // AJAX PARA VERIFICAR QUE NO SE UTILICE LA CATEGORÍA YA EN ALGUNA CARRERA
        try {
            const verificacion = await $.ajax({
                url: base_url + "Categorias/verificarUsoCategoria",
                type: "POST",
                data: { id: id },
                dataType: "json"
            });
            // SI ESTA USADA, NO SE PUEDE ELIMINAR LA CATEGORÍA
            if (verificacion.enUso) {
                toastr.error(
                    `No se puede eliminar: ${verificacion.mensaje} (Noticias afectadas: ${verificacion.totalNoticias})`, 
                    "Error",
                    {timeOut: 5000}
                );
                return;
            ...
            const toastConfirmacion = toastr.info(
                '<div class="text-center mb-2">¿Está seguro de eliminar esta categoría?</div>' +
            ...
            toastr.error("Error al verificar el estado de la categoría", "Error");
        }
    });
    ...
    async function eliminarCategoria(id) {
        try {
            // AJAX PARA ELIMINAR LA CATEGORÍA
            const respuesta = await $.ajax({
            ...
            // NOTIFICAR AL USUARIO DE QUE SE HA ELIMINADO LA CATEGORÍA
            $('#modalCategoria').modal('hide');
            resetearModalCategoria();
            toastr.success(
                respuesta.exito 
    ...
    ///////////////////////////////////////////////
    // EVENTO AL CAMBIAR EL FILTRO DE FECHA, PARA BUSCAR POR UNA FECHA EN CONCRETO
