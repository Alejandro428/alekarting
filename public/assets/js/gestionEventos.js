...
$('#fecha').datepicker({
    appendTo: '#modalEventos .modal-body', 
    showAnim: "slideDown",
    dateFormat: 'dd-mm-yy',
    showOtherMonths: true,
    selectOtherMonths: true,
...
// MÉTODO PARA CONFIGURAR EL SELECT DEL EMPLEADO DEL MODAL DE EVENTOS
function configurarSelect2Empleados(selector = '#empleados_id') {
    $(selector).select2({
        width: '100%',
        dropdownParent: $('#modalEventos .modal-content'),
...
$(document).on('click', '#btnAyudaEventos', function (event) {
  ...
  $('#modalAyudaGestionEventos').modal('show');
    ...
    // VALIDADOR PARA EL FORMULARIO DE TIPO DE EVENTO
    var formTipoEvento = new FormValidator('formTipoEvento', {
        nombre_tipo_evento: {
            required: true
        }
    });
    ...
    ////////////////////////////////////////
     ...
     // DESPLEGABLE DE de Filtros de Carreras
    ...
    // 2. DESPLEGABLE de Acciones de Carrera
    ...
    //         DATATABLES             //
    ///////////////////////////////////
    // CONFIGURACIÓN DEL DATATABLE PARA GESTIONAR LOS EVENTOS COMO ADMIN
    var datatable_eventosConfig = {
        ...
        columns: [
            ...
            { name: 'reservas', data: null, defaultContent: '' },  // Columna 5: Botón para Ver Reservas de evento
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    // 1. VERIFICAR SI EL EVENTO YA HA PASADO (fecha + hora_inicio)
        ...
        ajax: {
            // AJAX PARA OBTENER TODOS LOS EVENTOS QUE EXISTAN ACTUALES COMO ADMIN
            url: base_url + "EmpleadoEventos/obtenerTodosLosEventos",
            type: 'GET',
    ...
    function cargarEmpleados(selectId, idEmpleadoSeleccionado = null) {
        ...
        $.get(base_url + "Admin/getEmpleadosEventos", function(response) {
...
function cargarTiposEventoEnSelect(selectId, idTipoSeleccionado = null) {
    // AJAX PARA OBTENER TODOS LOS TIPOS DE EVENTOS DE LA PÁGINA 
...
//////////////////////////////////////
    ...
    function cargarEventos(selectId, idEventoSeleccionado = null) {
        $.get(base_url + "eventos/proximos", function(response) {
...
$(document).on('click', '.eliminarEvento', function(event) {
    ...
    $(document).on('click', '#btnnuevo', function(event) {
        ...
        configurarSelect2Empleados();
        
        // CARGAR EMPLEADOS DE EVENTOS
        cargarEmpleados('#empleados_id');
...
$(document).on('click', '#btnsalvar', async function (event) {
    ...
    var empleadoId = $('#formEvento').find('select[name="empleados_id"]').val().trim();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#modalEventos .modal-body #descripcion').summernote('code', evento.descripcion);
                    ...
                    configurarSelect2Empleados();
                    cargarEmpleados("#empleados_id", evento.empleados_id);
    ...
    ////////////////////////////////////////////
    //  INICIO ZONA TIPO EVENTO     //
    ///////////////////////////////////////////

     // VARIABLE GLOBAL PARA CONTROLAR EL MODO DEL TIPO DE EVENTO
var modoActual = "";

// MÉTODO PARA RESETEAR COMPLETAMENTE EL MODAL
function resetearModalTipoEvento() {
    $('#formTipoEvento')[0].reset();
    $('#formTipoEvento').find('input[name="id_tipo_evento"]').val("");
    $('#nombre_tipo_evento').removeClass('is-valid is-invalid');
    $('#divNombreTipoEvento').show();
    $('#divSelectTipoEvento').hide();
    $('#btnSalvarTipoEvento').show().text(' Guardar');
    $('#btnConfirmarEliminarTipo').hide();
    $('[data-bs-dismiss="modal"]').html('<i class="fas fa-times"></i> Cerrar');
}

///////////////////////////////////////
//    INICIO ZONA CREAR TIPO EVENTO  //
///////////////////////////////////////

// MÉTODO PARA ABRIR EL MODAL DE TIPO DE EVENTO
$(document).on('click', '#btnCrearTipoEvento', function (event) {
    event.preventDefault();
    modoActual = 'crear';
    resetearModalTipoEvento();
    $('#mdltituloTipoEvento').text('Nuevo registro de tipo de evento');
    $('#modalTipoEvento').modal('show');
});

///////////////////////////////////////
//    FIN ZONA CREAR TIPO EVENTO     //
///////////////////////////////////////

///////////////////////////////////////////
//  CHANGE PARA SELECT TIPO EVENTO      //
///////////////////////////////////////////

// EVENTO PARA CUANDO HAY CAMBIOS EN EL SELECT DE TIPO DE EVENTO
$(document).on('change', '#select_tipo_evento', function() {
    const opcion = $(this).find('option:selected');
    const id = opcion.val();
    const nombre = opcion.text();
    
    $('#formTipoEvento').find('input[name="id_tipo_evento"]').val(id);
    
    if (modoActual === 'editar') {
        if (id) {
            $('#divNombreTipoEvento').show();
            $('#nombre_tipo_evento').val(nombre);
        } else {
            $('#divNombreTipoEvento').hide();
...
///////////////////////////////////////
//    INICIO ZONA EDITAR TIPO EVENTO //
///////////////////////////////////////

// EVENTO DE CLICK PARA EL EDITAR TIPO DE EVENTO
$(document).on('click', '#btnEditarTipoEvento', function(evento) {
    evento.preventDefault();
    modoActual = 'editar';
    
    resetearModalTipoEvento();
    $('#mdltituloTipoEvento').text('Editar tipo de evento');
    $('#modalTipoEvento').modal('show');
    
    // Configuración específica para edición
    $('#divSelectTipoEvento').show();
    $('#divNombreTipoEvento').hide();
    
    cargarTiposEventoEnSelect('#select_tipo_evento');
});

///////////////////////////////////////
//    INICIO ZONA GUARDAR TIPO EVENTO//
///////////////////////////////////////

// MÉTODO PARA EL GUARDAR TIPO DE EVENTO
$(document).on('click', '#btnSalvarTipoEvento', async function(evento) {
    evento.preventDefault();
    
    const tipoEventoId = $('#formTipoEvento').find('input[name="id_tipo_evento"]').val();
    const tipoEventoNombre = ($('#nombre_tipo_evento').val() || '').trim();
    
    // Validación adicional para modo editar
    if (modoActual === 'editar' && !tipoEventoId) {
        toastr.error("Debe seleccionar un tipo de evento para editar", "Error");
        return;
    }

     // Validación del formulario
     const isFormValid = formTipoEvento.validateForm(evento);
     if (!isFormValid) {
         toastr.error("Por favor, corrija los errores en el formulario.", "Error de Validación");
         return;
     }
    ...
    try {
        const verificacion = await $.ajax({
            // AJAX PARA COMPROBAR QUE NO EXISTEN YA TIPOS DE EVENTO CON ESE NOMBRE
            url: base_url + "TiposEvento/verificarTipoEvento",
            type: "POST",
            data: {
                nombre_tipo_evento: tipoEventoNombre,
                id: tipoEventoId || null
            },
            dataType: "json"
        });
        
        if(verificacion.existe) {
            // SI YA EXISTE NO SE PUEDE CREAR/EDITAR ESE TIPO DE EVENTO
            toastr.error(verificacion.mensaje || "El tipo de evento ya existe", "Error");
            $('#nombre_tipo_evento').addClass('is-invalid');
            return;
        }
        
        // AJAX PARA CREAR/EDITAR EL TIPO DE EVENTO
        const respuesta = await $.ajax({
            url: tipoEventoId 
                ? base_url + "TiposEvento/editarTipoEvento" 
                : base_url + "TiposEvento/crearTipoEvento",
            type: "POST",
            data: {
                nombre_tipo_evento: tipoEventoNombre,
                id: tipoEventoId || null
        ...
        // SI LA RESPUESTA ES EXITOSA, SE MUESTRA UN TOAST DE ÉXITO
        if(respuesta.success) {
            ...
            toastr.success(
                tipoEventoId 
                    ...
                    : "¡Tipo de evento creado con éxito!", 
                "Éxito"
            );
        } else {
        ...
        if(error.status === 409) {
            toastr.error("Conflicto: " + (error.responseJSON?.mensaje || "El tipo de evento ya existe"), "Error");
            $('#nombre_tipo_evento').addClass('is-invalid');
        } else {
...
///////////////////////////////////////
//    FIN ZONA GUARDAR TIPO EVENTO  //
///////////////////////////////////////

///////////////////////////////////////
//    INICIO ZONA ELIMINAR TIPO EVENTO //
///////////////////////////////////////

// EVENTO PARA CUANDO SE SELECCIONA EL ELIMINAR TIPO DE EVENTO
$(document).on('click', '#btnEliminarTipoEvento', function(evento) {
    evento.preventDefault();
    modoActual = 'eliminar';
    
    resetearModalTipoEvento();

    $('#mdltituloTipoEvento').text('Eliminar tipo de evento');
    $('#modalTipoEvento').modal('show');
    ...
    $('#divSelectTipoEvento').show();
    $('#divNombreTipoEvento').hide();
    $('#btnSalvarTipoEvento').hide();
    $('#btnConfirmarEliminarTipo').show();
...
///////////////////////////////////////
//    INICIO ZONA CONFIRMAR ELIMINAR //
///////////////////////////////////////

// MÉTODO PARA CONFIRMAR EL ELIMINAR TIPO DE EVENTO
$(document).on('click', '#btnConfirmarEliminarTipo', async function() {
    const id = $('#formTipoEvento').find('input[name="id_tipo_evento"]').val();
    
    if (!id) {
        toastr.error("No se ha seleccionado ningún tipo de evento", "Error");
        return;
    }
    
    try {
        // AJAX PARA COMPROBAR QUE NO EXISTAN EVENTOS QUE UTILIZAN ESE TIPO DE EVENTO, EN ESE CASO, NO SE PODRÁ ELIMINAR EL TIPO DE EVENTO
        const verificacion = await $.ajax({
            url: base_url + "TiposEvento/verificarUsoTipoEvento",
            type: "POST",
            data: { id: id },
            dataType: "json"
        });
        ...
        if (verificacion.enUso) {
            // SI ESTA EN USO, NO SE PUEDE ELIMINAR
            toastr.error(
                `No se puede eliminar: ${verificacion.mensaje} (Eventos afectados: ${verificacion.totalEventos})`, 
                "Error",
                {timeOut: 5000}
            );
            return;
        ...
        const toastConfirmacion = toastr.info(
            '<div class="text-center mb-2">¿Está seguro de eliminar este tipo de evento?</div>' +
            '<div class="d-flex justify-content-center">' +
            '  <button class="btn btn-danger btn-sm mx-1" id="confirmarEliminarBtn">Sí, eliminar</button>' +
            '  <button class="btn btn-outline-secondary btn-sm mx-1" id="cancelarEliminarBtn">Cancelar</button>' +
            '</div>',
            "Confirmar eliminación",
            {
                ...
                onShown: function() {
                    $('#confirmarEliminarBtn').click(function() {
                        $('.toast-info').hide(); // Oculta todos los toasts de tipo info
                        $('.toast-info').remove(); // Elimina físicamente del DOM
                        
                        // Tu lógica de eliminación aquí
                        eliminarTipoEvento(id);
                    });
                    
                    $('#cancelarEliminarBtn').click(function() {
                        $('.toast-info').hide();
                        $('.toast-info').remove();
                    });
                }
        ...
        toastr.error("Error al verificar el estado del tipo de evento", "Error");
    }
});

// MÉTODO PARA ELIMINAR EL TIPO DE EVENTO
async function eliminarTipoEvento(id) {
    try {
        // AJAX PARA ELIMINAR EL TIPO DE EVENTO
        const respuesta = await $.ajax({
            url: base_url + "TiposEvento/eliminarTipoEvento",
            type: "POST",
            data: { id: id },
            dataType: "json"
        });
    ...
    // CONFIGURACIÓN DEL DATATABLE PARA GESTIONAR LAS RESERVAS DE EVENTOS DE LOS EVENTOS COMO ADMIN
    var datatable_reservaeventosConfig = {
        ...
        columns: [
            ...
            { name: 'pagado', data: 'pagado', defaultContent: '' },  // Columna 2: Botón para Pagar
            { name: 'eliminar', data: null, defaultContent: '' }   // Columna 3: Botón para Eliminar
        ],
        ...
        columnDefs: [
            ...
            {
                // Columna 1: NOMBRE DE USUARIO (recorta a 20 caracteres)
 ...
 function cargarReservasParaEvento(eventoId) {
    // ME GUARDO LA URL PARA HACER EL AJAX
    ...
    // HAGO UN AJAX CON ESA URL CUANDO EL ADMIN SELECCIONA UNO DE LOS BOTONES DE RESERVAS QUE EXISTEN
        ...
        if (response.success) {
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    // Mostrar Swal al enviar el correo correctamente
                    Swal.fire({
                        icon: 'success',
...
// MÉTODO PARA CONFIRMAR PAGO (NO PAGADO)
        ...
        if (result.isConfirmed) {
            ...
            $.post(base_url + "EmpleadoEventos/confirmarPago/" + idReserva)
                .done(function() {
                    ...
                    .done(function() {
                        // RECARGO LAS TABLAS Y INFORMO AL ADMIN
                        recargarTablas();
